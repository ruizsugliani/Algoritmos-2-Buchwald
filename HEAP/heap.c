 ...
 * *****************************************************************/
/*
bool _es_heap(int arr[], int pos, size_t n) {
    int hijo_izq = 2 * pos + 1;
    int hijo_der = 2 * pos + 2;
    //Caso base
    if (!arr[hijo_der] && !arr[hijo_izq]) return true;

    //Caso recursivo
    if (hijo_izq < n && arr[pos] > arr[hijo_izq]) return false;
    if (hijo_der < n && arr[pos] > arr[hijo_der]) return false;

    return _es_heap(arr, pos + 1, n);
}
